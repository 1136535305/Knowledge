...
class ThemeFragmentKotlin : Fragment() {
    ...
    private var mAdapter: ThemeListAdapter? = null
    private var mThemeBean: ZhihuThemeList.OthersBean? = null
    ...
    fun setDataSet(themeBean: ZhihuThemeList.OthersBean) {
        ...
        if (mDataListBuffer.containsKey(themeBean.id)) {               //查看相应的主题日报数据是否已缓存过
            ...
            recycler_view.scrollToPosition(0)                //复用fragment须复位
    ...
    private fun initData() {
        ApiManager.instance.createZhihuService().getThemeDetailById(mThemeBean!!.id)
                ...
                .subscribe {
                    ...
                    recycler_view.scrollToPosition(0)              //切换时回到顶部
